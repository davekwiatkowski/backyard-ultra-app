...
export const Statistics: FC<{ recentResultsCount: number, racesCount: number, countriesCount: number, mostRecentDate: string, resultsCount: number }> = (
    { recentResultsCount, mostRecentDate, countriesCount, racesCount, resultsCount }
) => {
    const { addSearchFilter } = useContext(ResultsContext);

    return <div className="stats stats-horizontal shadow">
        <div className="stat place-items-center">
            <div className="stat-title">Latest results</div>
            <div className="stat-value">{recentResultsCount}</div>
            <div className="stat-desc">
                Held on {' '}
                <button className="link link-primary" onClick={() => addSearchFilter('date', mostRecentDate)}>{mostRecentDate}</button>
            </div>
        </div>
        <div className="stat place-items-center">
