...
def create_site_data():
    ...
    df["distanceKm"] = pandas.to_numeric(
        df["distanceKm"].dropna().str.replace(" km", "")
    )
    ...
    df.loc[df["yards"] == 0, "yards"] = (
        (df["distanceKm"].dropna() / 6.7056).round(0).astype(int)
    )
    ...
    df["eventPlace"] = df["eventRankNoOverlap"].apply(
        lambda x: "W" if x == 1 else "A" if x == 2 else None
    )
    df["isTiedWin"] = df[df["eventPlace"] == "W"].duplicated(
        subset=["eventId", "eventPlace"], keep=False
    )
    ...
    df = df.sort_values(
        by=["yards", "eventPlace", "eventRankNoOverlap", "date"],
