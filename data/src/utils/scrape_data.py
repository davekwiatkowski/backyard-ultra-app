...
def write_results_for_events(limit=None):
    ...
    old_events_count = len(
        [event_id for event_id in df["EventId"] if should_skip_event_result(event_id)]
    )
    new_events = [
        event_id for event_id in df["EventId"] if not should_skip_event_result(event_id)
    ]
    if limit:
    ...
    print(
        f"Writing results for all new events ({events_count}) | Skipping over old events ({old_events_count})..."
    )
    with ThreadPoolExecutor() as threads:
        threads.map(lambda event_id: write_results_for_event(event_id), new_events)
    print(
        f"Finished writing results for events ({events_count}) in {(time.time() - start_time)} seconds."
    )
...
def write_results_for_event_list():
    ...
    while True:
        ...
        try:
            ...
            print(
                f"Finished writing results for event list in {(time.time() - start_time)} seconds."
            )
            break
